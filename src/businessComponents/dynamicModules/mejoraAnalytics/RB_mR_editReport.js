...
const RB_mR_addReport = () =>{
    ...
    const initialState = {
        ...
        widget: state?.bodyContents[0]?.widget
    };
    ...
    const onSubmit = async () => {
        ...
        const mandatoryItems = ["widget","reportName","query"]
        for(const item of mandatoryItems){
            if (advQuery.hasOwnProperty(item)) {}
            else {
                flag = false
                failureMessage="No items selected..."
            }
            if((advQuery[item]===null)||(advQuery[item]==="")){
                flag = false
        ...
        if(flag){
            ...
            const apiSendData = async () =>{
                const url = 'api/be/analytics/editReportAdvanced'
                const body = {
    ...
    const deletethis = () =>{
        ...
        if(confirmation){
            dispatch({type:'BACKDROP_ON'})
            const apiSendData = async () =>{
                const url = 'api/be/analytics/deleteReport'
                const body = {
                    "sid": state.loginData.sid,
                    "request": "DBA_Select",
                    "bu":state?.loginData?.identity?.buName ?? "",
    ...
    return <div style={{textAlign:'left'}}>
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div style={stdDiv}>
                <div>Widget Type</div>
                <select style={stdDiv2a} onChange={(e)=>handleAdvancedChange(e,'widget', 'string')}>
                    <option value={state?.bodyContents[0]?.widget} selected>{state?.bodyContents[0]?.widget}</option>
                    <option value="Bar Graph">Bar Graph</option>
                    <option value="Line Graph">Line Graph</option>
                    <option value="Area Graph">Area Graph</option>
                    <option value="Pie Chart">Pie Chart</option>
                    <option value="Counter Small">Counter Small</option>
                    <option value="Counter Medium">Counter Medium</option>
                    <option value="Counter Large">Counter Large</option>
                </select>
            </div>
