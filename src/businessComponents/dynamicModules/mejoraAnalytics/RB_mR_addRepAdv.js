...
const RB_mR_addRepAdv = () =>{
    ...
    const apiGetTables = (database) =>{
        ...
        const apiGetDatabasesCall = async () =>{
            const url = 'api/be/analytics/getTables'
            const body = {
    ...
    const apiGetDesc = (table) =>{
        dispatch({type:'BACKDROP_ON'})
        const apiGetDatabasesCall = async () =>{
            const url = 'api/be/analytics/getDesc'
            const body = {
                "sid": state.loginData.sid,
                "request": "DBA_Select",
                "bu":state?.loginData?.identity?.buName ?? "",
        ...
        if(mode==="Standard"){
            ...
            if(flag){
                ...
                const apiSendData = async () =>{
                    const url = 'api/be/analytics/newReport'
                    const body = {
                        "sid": state.loginData.sid,
                        "request": "DBA_Select",
                        "bu":state?.loginData?.identity?.buName ?? "",
            ...
            if(flag){
                dispatch({type:'BACKDROP_ON'})
                const apiSendData = async () =>{
                    const url = 'api/be/analytics/newReportAdvanced'
                    const body = {
                        "sid": state.loginData.sid,
                        "request": "DBA_Select",
                        "bu":state?.loginData?.identity?.buName ?? "",
