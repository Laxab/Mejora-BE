...
const LI_mR_reports = () =>{
    ...
    useEffect(()=>{
        ...
        const getListingData = async () =>{
            ...
            const url = 'api/be/reports/getReportNames'
            const body = {
    ...
    const onClickEvent = () =>{
        dispatch({type:"RIGHTBAR_ON",title:`Reports`, body:'RB_mR_addRepAdv', width:'500px',contents:'cols'})
    }
            ...
            data.map((item,index)=>(
                <div onClick={()=>clicked(item,index)} key={index} 
                    ...
                    <div style={{display:'flex',margin:"auto auto auto 5px",border:'0px solid red',textAlign:'left',flexDirection:'column',width:'80%'}}>
                        <div style={{position:'relative',top:'0px'}}><b>{item.dispName}</b></div>
                    </div>
                </div>
            ))
        }
